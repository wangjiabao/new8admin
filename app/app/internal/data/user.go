...
func (ub *UserBalanceRepo) RecommendLocationRewardNew8(ctx context.Context, userId int64, rewardAmount int64, rewardAmountUsdt int64) (int64, error) {
	var err error
	if err = ub.data.DB(ctx).Table("user_balance").
		Where("user_id=?", userId).
		Updates(map[string]interface{}{"balance_usdt": gorm.Expr("balance_usdt + ?", rewardAmountUsdt), "balance_dhb": gorm.Expr("balance_dhb + ?", rewardAmount), "recommend_location_total_usdt": gorm.Expr("recommend_location_total_usdt + ?", rewardAmountUsdt), "recommend_location_total": gorm.Expr("recommend_location_total + ?", rewardAmount)}).Error; nil != err {
		return 0, errors.NotFound("user balance err", "user balance not found")
	}
	...
	var userBalanceRecode UserBalanceRecord
	userBalanceRecode.Balance = rewardAmountUsdt
	userBalanceRecode.UserId = userBalance.UserId
	userBalanceRecode.Type = "reward"
	userBalanceRecode.CoinType = "usdt_dhb"
	userBalanceRecode.Amount = rewardAmount
	err = ub.data.DB(ctx).Table("user_balance_record").Create(&userBalanceRecode).Error
	if err != nil {
		return 0, err
	...
	var reward Reward
	reward.UserId = userBalance.UserId
	reward.Amount = rewardAmountUsdt
	reward.AmountB = rewardAmount
	reward.BalanceRecordId = userBalanceRecode.ID
